...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=1, embed_dim=768, norm_layer=None,
                 flatten=True):
        super().__init__()
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride, padding_mode='zeros')
...
class PaSST(nn.Module):
    ...
    def forward_features(self, x):
        ...
        freq_new_pos_embed = self.freq_new_pos_embed
        if x.shape[-2] < freq_new_pos_embed.shape[-2]:
            if self.training:
                toffset = torch.randint(1 + freq_new_pos_embed.shape[-2] - x.shape[-2], (1,)).item()
                if first_RUN: print(f" CUT with randomoffset={toffset} freq_new_pos_embed.shape",
                                    freq_new_pos_embed.shape)
                freq_new_pos_embed = freq_new_pos_embed[:, :, toffset:toffset + x.shape[-2], :]
            else:
                freq_new_pos_embed = freq_new_pos_embed[:, :, :x.shape[-2], :]
            if first_RUN: print(" CUT freq_new_pos_embed.shape", freq_new_pos_embed.shape)
        else:
            warnings.warn(
                f"the patches shape:{x.shape} are larger than the expected time encodings {freq_new_pos_embed.shape}, x will be cut")
            x = x[:, :, :freq_new_pos_embed.shape[-2], :]

        x = x + freq_new_pos_embed
...
def get_model(arch="passt_s_kd_p16_128_ap486", pretrained=True, n_classes=527, in_channels=1, fstride=10,
    ...
    elif arch == 'deit_tiny_distilled_patch16_224':
        model_func = deit_tiny_distilled_patch16_224
